...
10. [A schema-migration gotcha](#11-a-schema-migration-gotcha-if-paragraphs-suddenly-wont-load)
11. [Word-count guardrails and per-topic management](#12-word-count-guardrails-and-managing-paragraphs-per-topic)
12. [A LazyInitializationException gotcha](#14-a-lazyinitializationexception-gotcha)
13. [What's deliberately not built](#15-whats-deliberately-not-built)
...
## 11. A schema-migration gotcha (if paragraphs suddenly won't load)

If you ran an earlier version of this app *before* `Topic` became a database
table, your `paragraphs` table may still have rows from that era — Spring's
`ddl-auto=update` adds new columns (like `topic_id`) but never backfills or
migrates old data. Those old rows end up with `topic_id = NULL`, and every
paragraph on this list now has a real `Topic` behind it — a `null` there means
loading it throws, which used to take the *entire* admin paragraph list down
with it (a single bad row failing the whole request).

Two independent fixes:

- **`AdminParagraphService.toResponse()`** no longer assumes `getTopic()` is
  non-null — an orphaned row now renders as `"(no topic — from before topics
  existed; delete and re-add)"` instead of 500ing the whole list. You can see
  it and delete it from the UI (editing is blocked, since there's no valid
  topic to edit it into).
- **The admin tabs fetch independently.** `ParagraphsTab`/`TopicsTab` used to
  wrap their requests in `Promise.all`, which fails as a unit — meaning a
  broken paragraphs fetch was *also* wiping out an otherwise-successful topics
  fetch, which was the actual reason the topic dropdown stayed stuck on
  "Create a topic first" even after creating one. They use `Promise.allSettled`
  now, so each piece of data loads (or fails) on its own.

**To clear out old orphaned rows in one shot** instead of deleting them one at
a time from the UI, connect with `psql` and run:
```sql
TRUNCATE TABLE typing_results, paragraphs, topics RESTART IDENTITY CASCADE;
```
This only touches those three tables — your `users` table (and admin login)
is untouched. On the next backend restart, `DataSeeder` will find an empty
`paragraphs` table and reseed the 8 topics and 48 starter paragraphs fresh,
this time with `topic_id` set correctly from the start.

## 12. Word-count guardrails and managing paragraphs per topic

Two smaller additions:

- **`AdminParagraphService.WORD_LIMITS`** enforces a `[min, max]` word range
  per difficulty (EASY 15-40, MEDIUM 30-65, HARD 40-100 — loosely calibrated
  against the original seeded paragraphs), checked server-side in `create()`
  and `update()`. `GET /api/admin/paragraphs/word-limits` exposes the same
  numbers so the admin form can show live guidance (and a warning color)
  while typing, instead of only finding out on submit.
- **Multiple paragraphs per topic/difficulty already worked** —
  `getRandomParagraph()` always picked randomly among however many matched —
  but there was no way to *see* them grouped that way in the admin UI. A
  topic-filter row above the paragraph list (`ParagraphsTab.jsx`) now lets an
  admin narrow the list down to one topic at a time.

## 14. A LazyInitializationException gotcha

The Topic conversion introduced one more subtle bug, since fixed: `Paragraph.topic`
was originally mapped `@ManyToOne(fetch = FetchType.LAZY)`. That's normally fine —
except `application.properties` sets `spring.jpa.open-in-view=false` (deliberately;
leaving it on is a well-known anti-pattern), which means the Hibernate session
closes the instant a repository call returns. Three places read
`paragraph.getTopic().getName()` *after* that point — `ParagraphService.getRandomParagraph()`,
`AdminParagraphService.getAll()`, and `ResultService.saveResult()` — so all
three threw `LazyInitializationException` once real data existed, each one
surfacing to the client as the same generic `"Something went wrong."` (the
`GlobalExceptionHandler` catch-all didn't log anything server-side either,
so the real exception wasn't even visible in the console — also fixed, it
now logs the full stack trace before returning the generic client message).

Fixed by switching that one relationship to `FetchType.EAGER`. `Topic` is a
small lookup entity (just `id`/`name`/`icon`, no further relationships), so
always joining it in alongside its `Paragraph` is genuinely the right call
here — cheaper than reasoning about transaction boundaries at every call site,
and it can never throw `LazyInitializationException`.
